...
5. Dependencies`;
     ...
     */
    async analyzeAdvancedConflicts(dependencyContext, proposedUpdates) {
        try {
            ...
            const response = await this.analyzeWithLanguageModel(prompt, new vscode.CancellationTokenSource().token);
    ...
    buildAdvancedConflictPrompt(context, updates) {
...
[{"package": "name", "conflict": "description", "solution": "fix"}]`;
    ...
    parseSimpleConflictResponse(response) {
        try {
    ...
    async generateConflictResolution(conflictDetails, projectContext) {
        try {
     ...
     */
    buildConflictResolutionPrompt(conflictDetails, projectContext) {
...
## DEPENDENCY CHAINS:
${conflictDetails.dependencyChains?.map((chain) => `- ${chain}`).join('\n') || 'No chains provided'}
...
Focus on the specific packages mentioned and provide commands that will actually work for this exact scenario.`;
    ...
    async analyzeAnyDotnetError(errorOutput, commandContext) {
        try {
     ...
     */
    buildGenericErrorAnalysisPrompt(errorOutput, context) {
...
Analyze ALL error codes (NU1107, NU1102, NU1605, NU1608, MSB errors, etc.) and provide solutions specific to the exact error shown.`;
     ...
     */
    parseGenericErrorAnalysis(response) {
        try {
            const jsonMatch = response.match(/```json\n([\s\S]*?)\n```/);
            if (!jsonMatch) {
     ...
     */
    parseUnstructuredErrorResponse(response) {
        ...
        const recommendationLines = lines.filter(line => line.match(/^[-*]\s+/) ||
            line.match(/^\d+\.\s+/) ||
     ...
     */
    getFallbackErrorAnalysis(errorOutput) {
     ...
     */
    async analyzePackageFamilies(packageNames) {
        try {
     ...
     */
    async tryLanguageModelAnalysis(packageNames) {
        try {
            // Check if language model API is available
            if (!vscode.lm) {
     ...
     */
    async tryChatParticipantAnalysis(packageNames) {
        try {
     ...
     */
    buildPackageFamilyAnalysisPrompt(packageNames) {
...
- Provide clear reasoning for each grouping`;
     ...
     */
    generateFallbackPackageFamilyResponse(packageNames) {
        ...
        try {
            ...
            if (vscode.lm) {
                ...
                try {
                    ...
                    diagnosis.debugInfo['allModels'] = allModels.map(m => ({
                        id: m.id,
                        vendor: m.vendor,
                        family: m.family
                    }));
