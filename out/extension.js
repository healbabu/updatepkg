...
const dependencyGraphAnalyzer_1 = require("./services/dependencyGraphAnalyzer");
const upgradeStrategist_1 = require("./services/upgradeStrategist");
/**
...
async function activate(context) {
    ...
    async function handleSolutionUpdate(solutionPath, progress) {
        try {
            ...
            try {
                ...
                const dependencyAnalyzer = new dependencyGraphAnalyzer_1.DependencyGraphAnalyzer(logger, copilotService);
            ...
            const upgradeStrategist = new upgradeStrategist_1.UpgradeStrategist(logger, copilotService);
            ...
            progress.report({ message: '🎯 Generating upgrade strategies...' });
            const strategies = await upgradeStrategist.generateUpgradeStrategies(solutionPath, updatesMap, dependencyGraph);
        ...
        if (results.failures.length > 0) {
            ...
            for (const failure of results.failures) {
                ...
                if (failure.recommendations.length > 0) {
                    ...
                    failure.recommendations.forEach((rec, index) => {
                        panel.webview.postMessage({
                            type: 'log',
                            text: `   • ${index + 1}. ${rec}`
                        });
    ...
    function generateMarkdownSummary(results, conflictAnalysis) {
...
`;
                if (analysis.migrationSteps && analysis.migrationSteps.length > 0) {
                    analysis.migrationSteps.forEach((step, index) => {
...
`;
                if (analysis.testImpact && analysis.testImpact.length > 0) {
                    analysis.testImpact.forEach((impact) => {
                        markdown += `- ${impact}\n`;
                    });
                }
                else {
...
`;
    ...
    function findPackageFamily(packageName, families) {
 ...
 */
async function showStrategySelectionWebview(context, strategies, logger) {
    return new Promise((resolve) => {
        const panel = vscode.window.createWebviewPanel('upgradeStrategy', 'Choose Upgrade Strategy', vscode.ViewColumn.One, { enableScripts: true });
 ...
 */
function generateStrategySelectionHTML(strategies) {
    return `
    <!DOCTYPE html>
    <html>
    <head>
 ...
 */
async function executeUpgradeStrategy(strategy, solutionPath, progress, updatesMap, packageUpgrader, generateUpdateSummary) {
    ...
    const panel = vscode.window.createWebviewPanel('upgradeExecution', `Executing ${strategy.name}`, vscode.ViewColumn.One, { enableScripts: true });
    ...
    for (const phase of strategy.phases.sort((a, b) => a.order - b.order)) {
        ...
        for (const update of phase.packageUpdates) {
            try {
                ...
                await packageUpgrader.updatePackageWithDetails(update.packageName, update.recommendedVersion, updateProjectPath);
    ...
    try {
        ...
        if (error && typeof error === 'object' && 'details' in error) {
            const structuredError = error.details;
            panel.webview.postMessage({
                type: 'log',
            ...
            if (structuredError.conflictDetails) {
                ...
                if (conflict.dependencyChains && conflict.dependencyChains.length > 0) {
                    ...
                    conflict.dependencyChains.slice(0, 3).forEach((chain) => {
                        panel.webview.postMessage({
                            type: 'log',
                            text: `      ${chain}`
                        });
            ...
            if (structuredError.recommendations && structuredError.recommendations.length > 0) {
                ...
                structuredError.recommendations.slice(0, 5).forEach((rec, index) => {
                    panel.webview.postMessage({
                        type: 'log',
                        text: `      ${index + 1}. ${rec}`
                    });
 ...
 */
function generateEnhancedExecutionHTML(strategy) {
    return `
    <!DOCTYPE html>
    <html>
    <head>
...
function generateSmartActions(results) {
    const actions = [];
    if (results.failures.length === 0) {
        actions.push(`
