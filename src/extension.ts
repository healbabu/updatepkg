...
export async function activate(context: vscode.ExtensionContext) {
    ...
    async function handleProjectUpdate(projectPath: string, progress: vscode.Progress<{ message?: string }>) {
        ...
        if (breakingChangeFixes.length > 0) {
            vscode.window.showWarningMessage(
                `Agent found potential breaking changes:\n${breakingChangeFixes.map(fix => fix.description).join('\n')}`
            );
        }
        progress.report({ message: 'Checking for package updates...' });
        const updates = await packageUpgrader.checkForUpdates(projectPath) as any[]; // Will be PackageUpdateWithProject[]
        if (updates.length === 0) {
        ...
        const conflictAnalysis = new Map<string, any>();
        ...
        await showUpdateWebviewWithAnalysis(
            context,
            updatesMap,
            conflictAnalysis,
            logger,
            async (selected: { [project: string]: string[] }, panel: vscode.WebviewPanel) => {
                progress.report({ message: 'Applying package updates...' });
                const selectedUpdates = updates.filter((u: PackageUpdate) => selected[projectName]?.includes(u.packageName));
                let completed = 0;
    ...
    async function handleSolutionUpdate(solutionPath: string, progress: vscode.Progress<{ message?: string }>) {
        ...
        const projectItems = projects.map((projectPath: string) => ({
            label: path.basename(projectPath),
            description: projectPath,
            projectPath
        }));

        const selectedProjects = await vscode.window.showQuickPick<ProjectItem>(
            [
        ...
        const filteredUpdatesMap = new Map<string, PackageUpdate[]>();
        ...
        let conflicts = new Map<string, any>();
        ...
        await showUpdateWebviewWithAnalysis(
            ...
            async (selected: { [project: string]: string[] }, panel: vscode.WebviewPanel) => {
                ...
                const updateResults = {
                    ...
                    failures: [] as Array<{package: string, project: string, error: string, details?: string}>,
    ...
    async function findProjectFile(): Promise<string | undefined> {
        ...
        const picked = await vscode.window.showQuickPick(
            projectFiles.map(f => f.fsPath),
            { placeHolder: 'Select the project file to use' }
        );
        return picked;
    }
